...
describe('approveEscalation', () => {
  ...
  it('a same-mode request is a no-op grant and never asks (already at that floor)', async () => {
    const seen: unknown[] = []
    const spy = ingredients({ approver: approver('allowed-once', r => seen.push(r)) })
    await expect(approveEscalation(req({ requestedMode: 'read-only', effectiveMode: 'read-only' }), spy))
